...
theorem neg_neg (a : R) : - -a = a := by
  nth_rw 2 [← neg_add_cancel_left (-a) a]
...
theorem mul_inv_cancel (a : G) : a * a⁻¹ = 1 := by
  rw [← inv_mul_cancel (a⁻¹)]
  nth_rw 3 [← one_mul a⁻¹]
  rw [← inv_mul_cancel a, ← mul_assoc, ← mul_assoc, inv_mul_cancel, one_mul]

theorem mul_one (a : G) : a * 1 = a := by
  rw [← inv_mul_cancel a, ← mul_assoc, mul_inv_cancel, one_mul]

theorem mul_inv_rev (a b : G) : (a * b)⁻¹ = b⁻¹ * a⁻¹ := by
  rw [← one_mul (a * b)⁻¹, ← inv_mul_cancel b]
  nth_rw 2 [← one_mul b]
  rw [← inv_mul_cancel a, ← mul_assoc, ← mul_assoc, mul_assoc (b⁻¹ * a⁻¹),
